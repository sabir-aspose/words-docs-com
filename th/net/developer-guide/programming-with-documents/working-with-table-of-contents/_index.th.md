---
title: การทำงานกับสารบัญใน C#
second_title: Aspose.Words สำหรับ .NET
articleTitle: การทำงานกับสารบัญ
linktitle: การทำงานกับสารบัญ
description: "ฟิลด์สารบัญโดยละเอียด วิธีสร้างและแก้ไขฟิลด์ `TOC` โดยใช้ C# แทรก `TOC` C#"
type: docs
weight: 170
ai_search_scope: words_net
ai_search_endpoint: "https://docsearch.api.aspose.cloud/ask"
ai_search_fast_endpoint: "https://docsearch.api.aspose.cloud/search"
url: /th/net/working-with-table-of-contents/
...
คุณสามารถแทรกฟิลด์ `TOC` (สารบัญ) ลงในเอกสารที่ตำแหน่งปัจจุบันได้โดยการเรียกเมธอด [InsertTableOfContents](https://reference.aspose.com/words/net/aspose.words/documentbuilder/inserttableofcontents/)
...
|  สวิตช์ | คำอธิบาย |
|  :-  |  :-  |
| **Heading Styles**<br>*(&#92;O สวิตช์)* | <p>สวิตช์นี้กำหนดว่า `TOC` ควรสร้างขึ้นจากสไตล์หัวเรื่องที่มีอยู่แล้วภายใน ใน Microsoft Word สิ่งเหล่านี้ถูกกำหนดโดยหัวข้อ 1 – หัวข้อ 9 ใน Aspose.Words สไตล์เหล่านี้แสดงโดยการแจงนับ StyleIdentifier ที่สอดคล้องกัน การแจงนับนี้แสดงถึงตัวระบุโลแคลอิสระของสไตล์ เช่น `StyleIdentifier.Heading1` แสดงถึงสไตล์ของหัวข้อที่ 1 เมื่อใช้สิ่งนี้ การจัดรูปแบบและคุณสมบัติของสไตล์สามารถดึงข้อมูลจากคอลเลกชั่นสไตล์ของเอกสาร คลาส Style ที่สอดคล้องกันสามารถดึงข้อมูลจากคอลเลกชัน `Document.Styles` ได้โดยใช้คุณสมบัติที่จัดทำดัชนีของประเภท StyleIdentifier</p>
...
<p>- *&#92;L – กำหนดระดับใน `TOC` ฟิลด์ TC นี้จะปรากฏใน `TOC` ที่ใช้สวิตช์เดียวกันนี้จะรวมฟิลด์ TC นี้เท่านั้นหากอยู่ภายในช่วงที่ระบุ*</p>
...
คุณสามารถแทรกฟิลด์ TC ใหม่ที่ตำแหน่งปัจจุบันของ [DocumentBuilder](https://reference.aspose.com/words/net/aspose.words/documentbuilder/) ได้โดยการเรียกเมธอด [InsertField](https://reference.aspose.com/words/net/aspose.words/documentbuilder/insertfield/) และระบุชื่อฟิลด์เป็น "TC" พร้อมกับสวิตช์ใดๆ ที่จำเป็น
...
การใช้คลาส [Style](https://reference.aspose.com/words/net/aspose.words/style/) ที่ดึงมาสำหรับระดับ `TOC` เฉพาะที่คุณต้องการแก้ไข คุณยังสามารถแก้ไขลักษณะที่ปรากฏของสิ่งเหล่านี้ในเอกสารได้อีกด้วย หากต้องการเปลี่ยนลักษณะที่ปรากฏก่อนอื่น จะต้องเรียก [ParagraphFormat](https://reference.aspose.com/words/net/aspose.words/style/paragraphformat/) เพื่อดึงการจัดรูปแบบย่อหน้าสำหรับสไตล์ จากนี้ คุณสามารถดึงข้อมูลแท็บหยุดได้โดยการเรียก [TabStops](https://reference.aspose.com/words/net/aspose.words/paragraphformat/tabstops/) และแก้ไขแท็บหยุดที่เหมาะสม การใช้เทคนิคเดียวกันนี้ทำให้สามารถย้ายหรือลบแท็บทั้งหมดพร้อมกันได้
...
สารบัญสามารถลบออกจากเอกสารได้โดยการลบโหนดทั้งหมดที่พบระหว่างโหนด `FieldStart` และ FieldEnd ของฟิลด์ `TOC` รหัสด้านล่างแสดงให้เห็นถึงสิ่งนี้ การลบฟิลด์ `TOC` นั้นง่ายกว่าฟิลด์ปกติ เนื่องจากเราไม่ได้ติดตามฟิลด์ที่ซ้อนกัน แต่เราตรวจสอบว่าโหนด [FieldEnd](https://reference.aspose.com/words/net/aspose.words.fields/fieldend/) เป็นประเภท [FieldType.FieldTOC](https://reference.aspose.com/words/net/aspose.words.fields/fieldtype/) ซึ่งหมายความว่าเราพบจุดสิ้นสุดของ TOC ปัจจุบันแล้ว เทคนิคนี้สามารถใช้ได้ในกรณีนี้โดยไม่ต้องกังวลเกี่ยวกับฟิลด์ที่ซ้อนกัน เนื่องจากเราสามารถสรุปได้ว่าเอกสารที่มีรูปแบบเหมาะสมใดๆ จะไม่มีฟิลด์ `TOC` ที่ซ้อนกันโดยสมบูรณ์ภายในฟิลด์ `TOC` อื่น

ประการแรก โหนด [FieldStart](https://reference.aspose.com/words/net/aspose.words.fields/fieldstart/) ของแต่ละ `TOC` จะถูกรวบรวมและจัดเก็บ จากนั้น `TOC` ที่ระบุจะถูกแจกแจง ดังนั้นโหนดทั้งหมดภายในฟิลด์จึงถูกเยี่ยมชมและจัดเก็บ จากนั้นโหนดจะถูกลบออกจากเอกสาร
