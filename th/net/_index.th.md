---
title: Aspose.Words สำหรับ .NET
articleTitle: Aspose.Words สำหรับ .NET
linktitle: Aspose.Words สำหรับ .NET
description: "สร้าง แก้ไข แปลง เรนเดอร์ และพิมพ์เอกสารโดยใช้ C#"
type: docs
weight: 10
ai_search_scope: words_net
ai_search_endpoint: "https://docsearch.api.aspose.cloud/ask"
ai_search_fast_endpoint: "https://docsearch.api.aspose.cloud/search"
url: /th/net/
is_root: true
timestamp: 2024-09-25-11-08-55
---

<img src="/words/net/home_1" alt="aspose-words-net-home" align="left" style="width:110px; margin: 0 30px 30px 0"/>

**ยินดีต้อนรับสู่ Aspose.Words สำหรับ .NET!**

Aspose.Words สำหรับ .NET เป็นไลบรารีคลาสที่ช่วยให้แอปพลิเคชันของคุณสามารถดำเนินการประมวลผลเอกสารได้อย่างหลากหลาย

Aspose.Words รองรับรูปแบบเอกสารยอดนิยมส่วนใหญ่ เช่น DOC, DOCX, RTF, HTML, Markdown, PDF, XPS, EPUB และอื่นๆ

ด้วย Aspose.Words สำหรับ .NET คุณสามารถสร้าง แก้ไข แปลง แสดงผล และพิมพ์เอกสารโดยไม่ต้องใช้แอปพลิเคชันบุคคลที่สามหรือ Office Automation

------

<div class="row">
		<div class="col-md-4">
				<p><b>เกี่ยวกับ Aspose.Words</b></p>
						<hr>
						<p>ภาพรวม</p>
						<ul>
								<li><a href="/words/th/net/what-s-new-in-aspose-words-for-net/">มีอะไรใหม่ใน Aspose.Words สำหรับ .NET</a></li>
								<li><a href="/words/th/net/product-overview/">ภาพรวมผลิตภัณฑ์</a></li>
								<li><a href="/words/th/net/licensing/">การออกใบอนุญาตและการสมัครสมาชิก</a></li>
						</ul>
						<p>เริ่ม</p>
						<ul>
								<li><a href="/words/th/net/getting-started/">เริ่มต้นใช้งาน</a></li>
						</ul>
						<p>แหล่งข้อมูลที่เป็นประโยชน์</p>
						<ul>
								<li><a href="https://helpdesk.aspose.com/">Aspose.Words สำหรับฝ่ายช่วยเหลือสนับสนุนแบบชำระเงิน .NET</a></li>
								<li><a href="https://releases.aspose.com/words/net">ดาวน์โหลด Aspose.Words สำหรับ .NET</a></li>
								<li><a href="https://github.com/aspose-words/Aspose.Words-for-.NET">ดาวน์โหลดตัวอย่างได้ที่ GitHub Repository</a></li>
						</ul>
		</div>
		<div class="col-md-4">
				<p><b>เริ่ม</b></p>
						<hr>
						<p>เริ่มต้นใช้งาน</p>
						<ul>
								<li><a href="/words/th/net/supported-document-formats/">รูปแบบเอกสารที่รองรับ</a></li>
								<li><a href="/words/th/net/features/">คุณสมบัติที่รองรับ</a></li>
								<li><a href="/words/th/net/system-requirements/">ความต้องการของระบบ</a></li>
								<li><a href="/words/th/net/installation/">การติดตั้ง</a></li>
								<li><a href="https://www.nuget.org/packages/Aspose.Words/">ติดตั้ง Aspose.Words สำหรับ .NET NuGet Package</a></li>
						</ul>
						<p>เรียนรู้</p>
						<ul>
								<li><a href="/words/th/net/how-to-run-the-examples/">วิธีการเรียกใช้ตัวอย่าง</a></li>
								<li><a href="/words/th/net/how-to-run-aspose-words-in-docker/">วิธีเรียกใช้ Aspose.Words ในรูปแบบ Docker</a></li>
								<li><a href="/words/th/net/security/">หมายเหตุด้านความปลอดภัย</a></li>
						</ul>
		</div>
		<div class="col-md-4">
				<p><b>พัฒนาฟังก์ชั่น</b></p>
						<hr>
						<p>แนะนำ</p>
						<ul>
								<li><a href="/words/th/net/loading-saving-and-converting/">กำลังโหลด บันทึก และการแปลง</a></li>
								<li><a href="/words/th/net/rendering/">กำลังแสดงผล</a></li>
								<li><a href="/words/net/mail-merge-and-reporting/">Mail Merge และการรายงาน</a></li>
								<li><a href="/words/net/linq-reporting-engine/">เครื่องมือรายงาน LINQ</a></li>
								<li><a href="/words/th/net/print-a-document-programmatically-or-using-dialogs/">การพิมพ์เอกสารโดยทางโปรแกรมหรือใช้กล่องโต้ตอบ</a></li>
								<li><a href="/words/th/net/programming-with-documents/">การเขียนโปรแกรมด้วยเอกสาร</a></li>
								<li><a href="https://reference.aspose.com/words/net">คู่มือ Aspose.Words สำหรับ .NET API Reference</a></li>
						</ul>
						<p>เรียนรู้</p>
						<ul>
								<li><a href="https://releases.aspose.com/words/net/release-notes/">Aspose.Words สำหรับบันทึกประจำรุ่น .NET</a></li>
						</ul>
						<p>การสนับสนุนทางเทคนิค</p>
						<ul>
								<li><a href="https://forum.aspose.com/c/words/8">Aspose.Words สำหรับฟอรั่มสนับสนุน .NET ฟรี</a></li>
						</ul>
		</div>
</div>

------
