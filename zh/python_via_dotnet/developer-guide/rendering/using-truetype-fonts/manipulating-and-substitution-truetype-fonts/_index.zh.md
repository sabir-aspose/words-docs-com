...
所有可用的字体操作机制都包含在 [FontSettings](https://reference.aspose.com/words/python-net/aspose.words.fonts/fontsettings/) 类中。此类负责在定义的字体源中获取字体以及字体替换过程，如下所述。
...
由于上述过程非常耗时，因此可能会对首次启动时的应用程序性能产生负面影响。但是，[FontSettings](https://reference.aspose.com/words/python-net/aspose.words.fonts/fontsettings/) 的每个实例都有自己的缓存，这可以减少后续文档的处理时间。例如，您可以在不同文档之间共享 [FontSettings](https://reference.aspose.com/words/python-net/aspose.words.fonts/fontsettings/) 类的实例，这使您可以加快文档的加载速度。以下示例演示了这一点：
...
在未明确定义 [FontSettings](https://reference.aspose.com/words/python-net/aspose.words.fonts/fontsettings/) 的情况下，Aspose.Words 使用默认的 [FontSettings](https://reference.aspose.com/words/python-net/aspose.words.fonts/fontsettings/) 实例。该实例也会在文档之间自动共享，并且可以按如下方式提取：
...
如果您确定所有处理文档都需要相同的字体设置，那么建议设置并使用默认的 [FontSettings](https://reference.aspose.com/words/python-net/aspose.words.fonts/fontsettings/) 实例。假设您需要为所有文档使用相同的字体源。在这种情况下，您只需修改默认实例即可：
...
自定义 [FontSettings](https://reference.aspose.com/words/python-net/aspose.words.fonts/fontsettings/) 的优先级高于默认实例。
...
1. 如果 Aspose.Words 无法找到名称完全匹配的所需字体以及为此字体定义的 [alt_name](https://reference.aspose.com/words/python-net/aspose.words.fonts/fontinfo/alt_name/) 属性，则 Aspose.Words 将从指定字体信息的 [FontInfo](https://reference.aspose.com/words/python-net/aspose.words.fonts/fontinfo/) 类中查找使用 [alt_name](https://reference.aspose.com/words/python-net/aspose.words.fonts/fontinfo/alt_name/) 定义的字体。
1. 如果 Aspose.Words 无法找到已定义的字体，并且 [alt_name](https://reference.aspose.com/words/python-net/aspose.words.fonts/fontinfo/alt_name/) 也未定义，则将一一应用字体替换规则，如下所述（当找到合适的替换时，字体替换过程将停止，并且下一步不执行）：
...
{{< /highlight >}}
   1. 如果表格替换规则找不到字体，将应用 [FontInfo](https://reference.aspose.com/words/python-net/aspose.words.fonts/fontinfo/) 替换规则。该机制默认启用。 Aspose.Words根据特定文档中包含的字体信息找到最合适的字体。该信息可以从 [FontInfo](https://reference.aspose.com/words/python-net/aspose.words.fonts/fontinfo/) 类中获取，如下所示：<br>
{{< highlight python >}}
...
{{< /highlight >}}
      如果 [FontInfo](https://reference.aspose.com/words/python-net/aspose.words.fonts/fontinfo/) 不可用于丢失的字体，则该过程将停止。<br>
   1. 当`FontInfo`替换也失败时，将应用[DefaultFont](https://reference.aspose.com/words/python-net/aspose.words.fonts/defaultfontsubstitutionrule/)替换规则。该规则也是默认启用的。根据此规则，Aspose.Words 将尝试使用 [default_font_name](https://reference.aspose.com/words/python-net/aspose.words.fonts/defaultfontsubstitutionrule/default_font_name/) 属性中指定的默认字体。如果用户没有选择自己的默认字体，那么"Times New Roman"将被用作默认字体。可以禁用该规则，如下所示：<br>
{{< highlight python >}}
...
1. 最后，如果 Aspose.Words 在可用字体源中找不到任何字体，它会使用嵌入到 Aspose.Words 包中的免费 Fanwood 字体来呈现文档。<br>

如果 [FontInfo](https://reference.aspose.com/words/python-net/aspose.words.fonts/fontinfo/) 可用，*FontInfo 替换规则* 将始终解析字体并覆盖默认字体规则。如果您想使用默认字体规则，您应该禁用*FontInfo 替换规则*。请注意，*FontConfig 替换规则* 将在大多数情况下解析字体，从而覆盖所有其他规则。

## XML 中的字体回退设置

Aspose.Words 使用两种不同的机制--字体替换和字体回退。当在字体源中找不到文档中指定的字体时，使用字体替换，如上面各节所述。字体回退机制在字体解析时使用，但它不包含特定字符。在这种情况下，Aspose.Words 尝试使用该字符的后备字体之一。

有一种 [build_automatic](https://reference.aspose.com/words/python-net/aspose.words.fonts/fontfallbacksettings/build_automatic/) 方法可以通过扫描可用字体自动构建后备设置。由于此方法可能会产生非最佳回退设置，因此您可以使用 [FontFallbackSettings](https://reference.aspose.com/words/python-net/aspose.words.fonts/fontfallbacksettings/) 类的属性来控制字体回退行为。此类指定字体回退机制的设置。您可以按如下方式获取 [FontFallbackSettings](https://reference.aspose.com/words/python-net/aspose.words.fonts/fontfallbacksettings/) 类的实例：
...
Aspose.Words 为 Google Noto 字体提供预定义的字体后备设置。这些是根据 SIL 开放字体许可证获得许可的免费字体，可以从 Google Noto Fonts 下载。 [FontFallbackSettings](https://reference.aspose.com/words/python-net/aspose.words.fonts/fontfallbacksettings/) 类提供了 [load_noto_fallback_settings](https://reference.aspose.com/words/python-net/aspose.words.fonts/fontfallbacksettings/load_noto_fallback_settings/) 方法。它加载预定义的后备设置，该设置使用 Google Noto 字体，如下面的代码示例所示：
...
Aspose.Words 尝试自动在文件系统上查找 TrueType 字体。通常，您可以依靠 Aspose.Words 的默认行为来查找 `TrueType` 字体，但有时您需要指定自己的包含 TrueType 字体的文件夹。 [指定 TrueType 字体位置](/words/zh/python-net/specifying-truetype-fonts-location/) 主题描述了 Aspose.Words 查找字体的方式和位置，以及如何指定您自己的字体位置。
