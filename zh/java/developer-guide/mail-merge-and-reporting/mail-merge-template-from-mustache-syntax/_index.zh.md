---
title: Mustache语法中的Mail Merge模板
...
要澄清的第一个要点是Mustache不是模板引擎。 Mustache是另一种语法变体，可用于Aspose.Words支持的[加载格式](https://reference.aspose.com/words/java/com.aspose.words/loadformat/)中的任何模板。 因此，您可以以编程方式和通过接口创建这样的模板，您只需要包含某些语法并符合Mustache规范。
...
Mustache表示为无逻辑顺序，因为它缺少任何特定的控制流语句，如*for*循环和*if*和*else*条件。 但是您可以使用section tags processing lists和lambdas来实现条件求值和循环。 因此，要在Mail Merge操作中包含Mustache语法，您需要使用[UseNonMergeFields](https://reference.aspose.com/words/java/com.aspose.words/mailmerge/#getUseNonMergeFields)属性并将其值设置为*True*。
