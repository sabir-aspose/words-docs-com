...
* 文本由特殊字段节点–[FieldStart](https://reference.aspose.com/words/java/com.aspose.words/fieldstart/)和[FieldEnd](https://reference.aspose.com/words/java/com.aspose.words/fieldend/)包围
* [FieldSeparator](https://reference.aspose.com/words/java/com.aspose.words/fieldseparator/)节点将字段中的文本分隔为字段代码和字段结果
...
该结构也可以在下面使用演示项目**"DocumentExplorer"**以分层形式看到，该项目随**Aspose.Words**安装程序一起提供。
...
由于**ConvertFieldsToStaticText**方法接受两个参数–[CompositeNode](https://reference.aspose.com/words/java/com.aspose.words/compositenode/)属性和[FieldType](https://reference.aspose.com/words/java/com.aspose.words/fieldtype/)枚举，因此可以将任何复合节点传递给此方法。 这允许字段仅在文档的特定部分转换为静态文本。

例如，您可以传递一个[Document](https://reference.aspose.com/words/java/com.aspose.words/document/)对象并将指定类型的字段从整个文档转换为静态文本，或者您可以传递一个节的[Body](https://reference.aspose.com/words/java/com.aspose.words/body/)对象并仅转换在该正文中找到的字段。

{{% alert color="primary" %}}

传递块级节点（如[Paragraph](https://reference.aspose.com/words/java/com.aspose.words/paragraph/)）时，请注意，在某些情况下，字段可以跨越多个段落。 如果发生这种情况，建议改为传递复合的父级以避免这种情况。

{{% /alert %}}

传递给**ConvertFieldsToStaticText**方法的[FieldType](https://reference.aspose.com/words/java/com.aspose.words/fieldtype/)枚举指定应将哪种类型的字段转换为静态文本。 在文档中找到的任何其他字段类型将保持不变。
