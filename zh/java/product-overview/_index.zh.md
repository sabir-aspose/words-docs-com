...
Aspose.Words被设计为在服务器或客户端上执行很大。 Aspose.Words是单个Javajar，只需复制即可与任何Java应用程序一起部署。 如果需要，您还可以为高级字体功能添加`HarfBuzz`插件（在单独的jar中发布）。 标准JavaImageIOJAI扩展也可以添加到您的平台，以支持额外的ImageIO编解码器（在现代Java环境中不需要）。
