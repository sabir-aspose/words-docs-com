...
几乎您想要使用 Aspose.Words 执行的任何任务都涉及加载文档。 `Document` 类表示加载到内存中的文档。该文档有几个重载的构造函数，允许您创建空白文档或从文件或流加载它。文档可以以 Aspose.Words 支持的任何加载格式加载。有关所有支持的加载格式的列表，请参阅 [LoadFormat](https://reference.aspose.com/words/zh/net/aspose.words/loadformat/) 枚举。

## 创建新文档 {#create-a-new-document}

我们将调用不带参数的 [Document](https://reference.aspose.com/words/zh/net/aspose.words/document/document/) 构造函数来创建一个新的空白文档。如果要以编程方式生成文档，最简单的方法是使用 [DocumentBuilder](https://reference.aspose.com/words/zh/net/aspose.words/documentbuilder/) 类添加文档内容。
...
- 文档纸张尺寸为 [PaperSize](https://reference.aspose.com/words/zh/net/aspose.words/papersize/).**Letter**。

{{% /alert %}}

## 加载文档 {#load-a-document}

要加载任何 [LoadFormat](https://reference.aspose.com/words/zh/net/aspose.words/loadformat/) 格式的现有文档，请将文件名或流传递到文档构造函数之一。加载文档的格式由其扩展名自动确定。
