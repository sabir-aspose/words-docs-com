...
Microsoft Word生成并保存预渲染的绘图以及`SmartArt`对象。 在大多数情况下，预渲染的绘图由Aspose.Words很好地呈现，并且不需要额外的操作。 但是，如果文档由其他应用程序保存，则预呈现的SmartArt绘图可能丢失或不正确。 在这种情况下，`SmartArt`对象本身应该使用Aspose.Words进行布局和呈现。 我们将此过程称为`SmartArt`冷渲染。
...
* 如果存在预渲染的绘图但不正确，则需要通过调用[UpdateSmartArtDrawing](https://reference.aspose.com/words/cpp/aspose.words.drawing/shape/updatesmartartdrawing/)方法显式执行SmartArt冷渲染。
...
目前，仅支持有限数量的标准Microsoft WordSmartArt布局。 此外，其中一些布局部分支持，这意味着显着节点和图表的形状被呈现，但Microsoft Word和Aspose.Words图表布局之间可能存在差异。
...
| `Relationship` | <ul><li>平衡表</li><li>漏斗,漏斗</li><li>齿轮</li><li>加减</li><li>箭头色带</li><li>平衡箭头</li><li>反对箭头</li><li>嵌套目标</li><li>基本目标</li><li>基本Pie</li><li>基本维恩</li><li>堆叠维恩</li><li>互连环</li></ul> | <ul><li>圈关系</li><li>反对的想法</li><li>方程</li><li>垂直方程</li><li>线性维恩</li></ul> |
| `Matrix` | <ul><li>基本矩阵</li><li>标题为矩阵</li><li>网格矩阵</li><li>周期矩阵</li></ul> |  |
