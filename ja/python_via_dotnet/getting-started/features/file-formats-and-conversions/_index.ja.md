...
linktitle: ファイル形式と変換
description: "わずか 2 行の Python コードで、サポートされている読み込み形式から保存形式にドキュメントを変換します。高品質の変換を提供します。"
type: docs
weight: 40
...
ドキュメントのロード、変換、保存の詳細については、「[ロード、保存、変換](/words/ja/python-net/loading-saving-and-converting/)」セクションを参照してください。
...
このような状況では、Aspose.Words では変換に「高忠実度」のマークが付けられ、出力ドキュメントは Microsoft Word 経由で変換した後と同じになることに注意してください。
...
スケーラブルなサーバー側シナリオでは、主にそのシンプルでクリーンな設計のために Aspose.Words が使用されます。Aspose.Words はマルチスレッドをサポートしています。 [Document](https://reference.aspose.com/words/python-net/aspose.words/document/) オブジェクトは互いに独立しており、一度に 1 つのスレッドのみがドキュメントを変更できる限り (コレクション クラスの一般的な要件)、複数のスレッドで任意の数のドキュメントを同時に処理できます。
...
何度かお客様から、Aspose.Words の速度が信じられないという報告を受けました。彼らは、巨大なドキュメントをデバッガにロードするコード行をまたいで実行しましたが、その作業は非常に瞬時で、ファイルが [Document](https://reference.aspose.com/words/python-net/aspose.words/document/) オブジェクトにロードされたことが信じられないほどでした。
