...
linktitle: OOXMLチャートをゼロから作成する
description: "C++を使用してドキュメントに異なるグラフを作成する方法。"
type: docs
weight: 10
...
Aspose.Wordsは`DocumentBuilder`クラスに追加された`InsertChart`メソッドを提供します。 それでは、`DocumentBuilder->InsertChart`メソッドを使用して簡単な縦棒グラフを文書に挿入する方法を見てみましょう:

### 縦棒グラフの挿入方法

以下のコード例は、縦棒グラフを挿入する方法を示しています。
...
### どのようにInsertScatterChart

以下のコード例は、散布図を挿入する方法を示しています。
...
以下のコード例は、バブルチャートを挿入する方法を示しています。
