---
title: Lavorare con il sommario in C#
second_title: Aspose.Words per .NET
articleTitle: Lavorare con il sommario
linktitle: Lavorare con il sommario
description: "Campo Sommario in dettaglio. Come creare e modificare il campo `TOC` utilizzando C#. Inserisci `TOC` C#."
type: docs
weight: 170
ai_search_scope: words_net
ai_search_endpoint: "https://docsearch.api.aspose.cloud/ask"
ai_search_fast_endpoint: "https://docsearch.api.aspose.cloud/search"
url: /it/net/working-with-table-of-contents/
...
Nell'esempio di codice seguente viene illustrato come inserire un sommario (TOC) in un documento utilizzando gli stili di titolo come voci.
...
La prima chiamata a **UpdateFields** creerà il `TOC`, tutte le voci di testo verranno popolate e il `TOC` apparirà quasi completo. Mancano solo i numeri di pagina che per ora vengono visualizzati con "?". La seconda chiamata a **UpdatePageLayout** costruirà il layout del documento in memoria. Questo è necessario per raccogliere i numeri di pagina delle voci. I numeri di pagina corretti calcolati da questa chiamata vengono quindi inseriti nel sommario.
...
Spesso una riga di testo specifica è designata per il `TOC` ed è contrassegnata da un campo `TC`. Il modo più semplice per farlo in Microsoft Word è evidenziare il testo e premere *ALT+SHIFT+O*. Questo crea automaticamente un campo `TC` utilizzando il testo selezionato. La stessa tecnica può essere realizzata tramite codice. Il codice seguente troverà il testo corrispondente all'input e inserirà un campo `TC` nella stessa posizione del testo. Il codice si basa sulla stessa tecnica utilizzata nell'articolo.

Il seguente esempio di codice mostra come trovare e inserire un campo `TC` nel testo in un documento:
...
Il seguente esempio di codice modifica una proprietà di formattazione utilizzata nello stile `TOC` di primo livello:
