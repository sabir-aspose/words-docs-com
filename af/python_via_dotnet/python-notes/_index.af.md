...
linktitle: Python En .NET Weergawe Verskille
description: "Aspose.Words vir Python via .NET is'n verpakking van Aspose.Words vir .NET, hierdie bladsy beskryf die verskille in kenmerke en API van hierdie twee produkte."
type: docs
weight: 15
...
* [DocumentVisitor](https://reference.aspose.com/words/python-net/aspose.words/documentvisitor/) implementering is tans nie moontlik vanaf die Python kode nie.
...
Basis klas vir alle dokument nodes in Aspose.Words DOM is [Node](https://reference.aspose.com/words/python-net/aspose.words/node/) klas. Byvoorbeeld [get_child](https://reference.aspose.com/words/python-net/aspose.words/compositenode/get_child/) metode terugkeer en geval van [Node](https://reference.aspose.com/words/python-net/aspose.words/node/) klas, maar as jy nodig het om die teruggekeer node verander, in die meeste gevalle moet jy dit te gooi om konkrete tipe. Die volgende kode demonstreer hoe om die lettertipe kleur van die eerste [Run](https://reference.aspose.com/words/python-net/aspose.words/run/) in die dokument te verander:
...
Giet kan ook nodig wees wanneer [clone](https://reference.aspose.com/words/python-net/aspose.words/document/clone/) metode gebruik word:
...
Soos jy dalk opgemerk het waar in C# kode jy `(Paragraph)node` sou gebruik vir giet, in Python moet jy `node.as_paragraph()` metode gebruik. In die Python weergawe van Aspose.Words [Node](https://reference.aspose.com/words/python-net/aspose.words/node/) klas stel die volgende skakel van `as_xxx` metodes:
...
Soos jy dalk opgemerk [Field](https://reference.aspose.com/words/python-net/aspose.words.fields/field/) voorwerp bied ook stel van `as_xxx` metodes, wat hieronder gelys:
...
[FontSourceBase](https://reference.aspose.com/words/python-net/aspose.words.fonts/fontsourcebase/) klas bied stel van `as_xxx` metodes, wat hieronder gelys word:
...
## API Lede Se Naam
Om nader aan Python wêreld te wees, API lede van Aspose.Words vir Python via .NET gebruik pythonic slang styl, maar in die meeste gevalle het hulle een tot een analoog in Aspose.Words vir .NET API. Jy kan hierdie analoog in die [xml-lêer](wrapper.zip).
