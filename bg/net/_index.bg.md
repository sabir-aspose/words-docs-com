---
title: Aspose.Words вместо .NET
articleTitle: Aspose.Words вместо .NET
linktitle: Aspose.Words вместо .NET
description: "Генериране, промяна, преобразуване, предаване и печат на документи, използвайки C#."
type: docs
weight: 10
ai_search_scope: words_net
ai_search_endpoint: "https://docsearch.api.aspose.cloud/ask"
ai_search_fast_endpoint: "https://docsearch.api.aspose.cloud/search"
url: /bg/net/
is_root: true
timestamp: 2024-09-25-11-08-55
---

<img src="/words/net/home_1" alt="aspose-words-net-home" align="left" style="width:110px; margin: 0 30px 30px 0"/>

**Добре дошли в Aspose.Words вместо .NET!**

Aspose.Words вместо .NET е клас библиотека, която позволява вашите приложения да изпълняват голям набор от задачи по обработка на документи.

Aspose.Words поддържа повечето от популярните формати на документи като DOC, DOX, RTF, HTML, Markdown, PDF, XPS, EPUB и други.

С Aspose.Words вместо .NET, можете да генерирате, модифицирате, конвертирате, правите и печатате документи без приложения на трети страни или Office Automation.

------

<div class="row">
	<div class="col-md-4">
		<p><b>Относно Aspose.Words</b></p>
			<hr/><p>ПРЕВОД</p>
			<ul>
				<li><a href="/words/bg/net/what-s-new-in-aspose-words-for-net/">Какво ново в Aspose.Words вместо .NET</a></li>
				<li><a href="/words/bg/net/product-overview/">Преглед на продукта</a></li>
				<li><a href="/words/bg/net/licensing/">Лицензиране и записване</a></li>
			</ul>
			<p>Започвай.</p>
			<ul>
				<li><a href="/words/bg/net/getting-started/">Започване</a></li>
			</ul>
			<p>ИЗПОЛЗВАЩИ РЕСУРСИ</p>
			<ul>
				<li><a href="https://helpdesk.aspose.com/">Aspose.Words вместо .NET Платено подпомагане</a></li>
				<li><a href="https://releases.aspose.com/words/net">Изтегляне Aspose.Words вместо .NET</a></li>
				<li><a href="https://github.com/aspose-words/Aspose.Words-for-.NET">Изтегляне на примери в GitHub Хранилище</a></li>
			</ul>
	</div>
	<div class="col-md-4">
		<p><b>Започвай.</b></p>
			<hr/><p>ЗАПОЧВАНЕ</p>
			<ul>
				<li><a href="/words/bg/net/supported-document-formats/">Поддържани формати на документи</a></li>
				<li><a href="/words/bg/net/features/">Поддържани функции</a></li>
				<li><a href="/words/bg/net/system-requirements/">Системни изисквания</a></li>
				<li><a href="/words/bg/net/installation/">Монтаж</a></li>
				<li><a href="https://www.nuget.org/packages/Aspose.Words/">Инсталиране Aspose.Words вместо .NET NuGet Package</a></li>
			</ul>
			<p>НАУЧИ</p>
			<ul>
				<li><a href="/words/bg/net/how-to-run-the-examples/">Как да използваме примерите</a></li>
				<li><a href="/words/bg/net/how-to-run-aspose-words-in-docker/">Как да бягаме Aspose.Words в Docker</a></li>
				<li><a href="/words/bg/net/security/">Бележки по сигурността</a></li>
			</ul>
	</div>
	<div class="col-md-4">
		<p><b>Разработване на функции</b></p>
			<hr/><p>ГВИДЕ</p>
			<ul>
				<li><a href="/words/bg/net/loading-saving-and-converting/">Зареждане, спестяване и преобразуване</a></li>
				<li><a href="/words/bg/net/rendering/">Внасяне</a></li>
				<li><a href="/words/net/mail-merge-and-reporting/">Mail Merge и докладване</a></li>
				<li><a href="/words/net/linq-reporting-engine/">LINQ Докладващ двигател</a></li>
				<li><a href="/words/bg/net/print-a-document-programmatically-or-using-dialogs/">Печат на документ Програмиране или използване на диалогови файлове</a></li>
				<li><a href="/words/bg/net/programming-with-documents/">Програмиране с документи</a></li>
				<li><a href="https://reference.aspose.com/words/net">Aspose.Words вместо .NET API Reference Ръководство</a></li>
			</ul>
			<p>НАУЧИ</p>
			<ul>
				<li><a href="https://releases.aspose.com/words/net/release-notes/">Aspose.Words вместо .NET Бележки за освобождаване</a></li>
			</ul>
			<p>ТЕХНИЧЕСКИ ПОДКРЕПА</p>
			<ul>
				<li><a href="https://forum.aspose.com/c/words/8">Aspose.Words вместо .NET Форум за безплатна подкрепа</a></li>
			</ul>
	</div>
</div>

------
