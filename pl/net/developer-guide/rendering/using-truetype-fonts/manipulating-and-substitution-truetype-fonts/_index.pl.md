---
title: Manipuluj i zastępuj czcionki TrueType w C#
second_title: Aspose.Words dla .NET
articleTitle: Manipuluj i zastępuj czcionki TrueType
linktitle: Manipuluj i zastępuj czcionki TrueType
description: "Aspose.Words dla .NET może osadzić prawidłowe czcionki TrueType w dokumencie wynikowym, aby zapewnić jego dokładne wyświetlanie w formacie C#. Jeśli czcionka lub określony znak nie jest dostępny, Aspose.Words wyszukuje odpowiedni zamiennik czcionki lub korzysta z mechanizmu zastępczego czcionki."
type: docs
weight: 10
...
Wszystkie dostępne mechanizmy manipulacji czcionkami zawarte są w klasie [FontSettings](https://reference.aspose.com/words/net/aspose.words.fonts/fontsettings/). Klasa ta odpowiada za pobieranie czcionek ze zdefiniowanych źródeł czcionek oraz za proces zastępowania czcionek, jak opisano poniżej.
...
Niestandardowy **FontSettings** ma wyższy priorytet niż instancja domyślna.
...
1. Jeżeli Aspose.Words nie jest w stanie zlokalizować zdefiniowanej czcionki i nie zdefiniowano również **AltName**, wówczas stosowane są zasady podmiany czcionek jedna po drugiej, jak opisano poniżej (po znalezieniu odpowiedniego zamiennika, Proces Podstawiania Czcionek zostaje zatrzymany, a następny krok nie jest wykonywany):
   ...
   1. Reguła podstawiania **FontInfo** zostanie zastosowana w przypadku, gdy reguła podstawiania tabeli nie znajdzie czcionki. Mechanizm ten jest domyślnie włączony. Aspose.Words wyszukuje najbardziej odpowiednią czcionkę na podstawie informacji o czcionce zawartych w konkretnym dokumencie. Informacje te można uzyskać z klasy **FontInfo**, jak pokazano poniżej:<br>
      ...
      **.NET**<br>
      ...
      {{< /highlight >}}
      Jeśli dla brakującej czcionki **FontInfo** nie jest dostępny, proces zostaje zatrzymany.<br>
   1. Zasada podstawienia **DefaultFont** będzie stosowana w przypadku, gdy również nie powiodła się podstawienie `FontInfo`. Ta reguła jest również domyślnie włączona. Zgodnie z tą zasadą, Aspose.Words będzie próbował użyć domyślnej czcionki określonej we właściwości [DefaultFontName](https://reference.aspose.com/words/net/aspose.words.fonts/defaultfontsubstitutionrule/defaultfontname/). Jeśli użytkownik nie wybrał własnej czcionki domyślnej, jako czcionka domyślna zostanie użyta czcionka "Times New Roman". Regułę tę można wyłączyć w sposób pokazany poniżej:<br>
      **.NET**<br>
      {{< highlight csharp >}}
...
1. Na koniec, jeśli Aspose.Words nie może znaleźć żadnej czcionki wśród dostępnych źródeł czcionek, renderuje dokument przy użyciu darmowej czcionki Fanwood osadzonej w zestawie Aspose.Words.<br>
...
## Jak rozpoznać, że czcionka została wymieniona

Czasami może nie być jasne, dlaczego układ dokumentu się zmienił lub dlaczego niektóre czcionki nie wyglądają zgodnie z oczekiwaniami. W takich przypadkach na ratunek przychodzą komunikaty ostrzegające o podmianie czcionek, implementowane przez interfejs [IWarningCallback](https://reference.aspose.com/words/net/aspose.words/iwarningcallback/). Mają typ ostrzeżenia [FontSubstitution](https://reference.aspose.com/words/net/aspose.words/warningtype/) i standardowy format tekstu opisu "Czcionka"<OriginalFont> ' nie odnaleziono. Za pomocą '<SubstitutionFont> ' zamiast tego czcionka. Powód:<Reason> ", z następujących powodów:

- "nazwa alternatywna z dokumentu" – do zastąpienia przez [AltName](https://reference.aspose.com/words/net/aspose.words.fonts/fontinfo/properties/altname)
- "podstawienie czcionki" – dla podstawienia według reguły konfiguracji czcionki
- "podstawienie tabeli" – dla podstawienia według reguły tabeli
- "podstawienie informacji o czcionce" – dla podstawienia według reguły informacji o czcionce
- "domyślne podstawianie czcionek" – dla reguły podstawiania czcionek domyślnych
...
Istnieje metoda [BuildAutomatic](https://reference.aspose.com/words/net/aspose.words.fonts/fontfallbacksettings/buildautomatic/), która automatycznie tworzy ustawienia awaryjne, skanując dostępne czcionki. Ponieważ ta metoda może generować nieoptymalne ustawienie zastępcze, można kontrolować zachowanie zastępczej czcionki, korzystając z właściwości klasy [FontFallbackSettings](https://reference.aspose.com/words/net/aspose.words.fonts/fontfallbacksettings/). Ta klasa określa ustawienia mechanizmu rezerwowego czcionek. Instancję klasy **FontFallbackSettings** można uzyskać w następujący sposób:
...
Aspose.Words zapewnia predefiniowane ustawienia zastępczych czcionek dla czcionek Google Noto. Są to bezpłatne czcionki objęte licencją SIL Open Font License, które można pobrać ze strony Google Noto Fonts. Klasa **FontFallbackSettings** udostępnia metodę [LoadNotoFallbackSettings](https://reference.aspose.com/words/net/aspose.words.fonts/fontfallbacksettings/loadnotofallbacksettings/). Ładuje predefiniowane ustawienia zastępcze, które używają czcionek Google Noto, jak pokazano w przykładzie kodu poniżej:
...
Aspose.Words próbuje automatycznie znaleźć czcionki TrueType w systemie plików. Zwykle w celu znalezienia czcionek `TrueType` można polegać na domyślnym zachowaniu Aspose.Words, ale czasami trzeba określić własne foldery zawierające czcionki TrueType. Temat [Określ lokalizację czcionek TrueType](/words/pl/net/specifying-truetype-fonts-location/) opisuje, jak i gdzie Aspose.Words szuka czcionek, a także jak określić własne lokalizacje czcionek.
