---
title: Làm việc với Mục lục
second_title: Aspose.Words cho Python via .NET
articleTitle: Làm việc với Mục lục
linktitle: Làm việc với Mục lục
description: "Chèn và quản lý 'Mục lục' trong tài liệu bằng Python."
type: docs
weight: 170
...
Bạn có thể chèn trường `TOC` (mục lục) vào tài liệu ở vị trí hiện tại bằng cách gọi phương thức [DocumentBuilder.insert_table_of_contents](https://reference.aspose.com/words/python-net/aspose.words/documentbuilder/insert_table_of_contents/).

Mục lục trong tài liệu Word có thể được xây dựng theo nhiều cách và được định dạng bằng nhiều tùy chọn khác nhau. Các chuyển đổi trường mà bạn chuyển đến phương thức sẽ kiểm soát cách tạo và hiển thị bảng trong tài liệu của bạn.
...
Đoạn mã này thể hiện mục lục mới được chèn vào một tài liệu trống. Sau đó, lớp [DocumentBuilder](https://reference.aspose.com/words/python-net/aspose.words/documentbuilder/) được sử dụng để chèn một số định dạng nội dung mẫu với các kiểu tiêu đề thích hợp được sử dụng để đánh dấu nội dung sẽ được đưa vào TOC. Các dòng tiếp theo sẽ điền `TOC` bằng cách cập nhật các trường và bố cục trang của tài liệu.
...
Lệnh gọi đầu tiên tới [Document.update_fields](https://reference.aspose.com/words/python-net/aspose.words/document/update_fields/) sẽ tạo `TOC`, tất cả các mục nhập văn bản đều được điền và `TOC` có vẻ gần như hoàn chỉnh. Điều duy nhất còn thiếu là số trang hiện được hiển thị bằng "?". Lệnh gọi [Document.update_page_layout](https://reference.aspose.com/words/python-net/aspose.words/document/update_page_layout/) thứ hai sẽ xây dựng bố cục của tài liệu trong bộ nhớ. Điều này cần phải được thực hiện để thu thập số trang của các mục. Sau đó, số trang chính xác được tính toán từ lệnh gọi này sẽ được chèn vào TOC.
...
Bạn có thể chèn trường TC mới vào vị trí hiện tại của [DocumentBuilder](https://reference.aspose.com/words/python-net/aspose.words/documentbuilder/) bằng cách gọi phương thức [DocumentBuilder.insert_field](https://reference.aspose.com/words/python-net/aspose.words/documentbuilder/insert_field/) và chỉ định tên trường là "TC" cùng với bất kỳ công tắc nào cần thiết. Ví dụ dưới đây cho thấy cách chèn trường `TC` vào tài liệu bằng [DocumentBuilder](https://reference.aspose.com/words/python-net/aspose.words/documentbuilder/).
...
Định dạng của các mục trong `TOC` không sử dụng kiểu gốc của các mục được đánh dấu, thay vào đó, mỗi cấp độ được định dạng bằng kiểu `TOC` tương đương. Ví dụ: cấp độ đầu tiên trong `TOC` được định dạng theo kiểu **TOC1**, cấp độ thứ hai được định dạng theo kiểu **TOC2**, v.v. Điều này có nghĩa là để thay đổi giao diện của `TOC`, các kiểu này phải được sửa đổi. Trong Aspose.Words, các kiểu này được biểu thị bằng [StyleIdentifier.TOC1](https://reference.aspose.com/words/python-net/aspose.words/styleidentifier/#toc1) độc lập của miền địa phương cho đến [StyleIdentifier.TOC9](https://reference.aspose.com/words/python-net/aspose.words/styleidentifier/#toc9) và có thể được truy xuất từ bộ sưu tập [Document.styles](https://reference.aspose.com/words/python-net/aspose.words/documentbase/styles/) bằng cách sử dụng các mã định danh này.

Sau khi đã lấy được kiểu thích hợp của tài liệu, định dạng cho kiểu này có thể được sửa đổi. Mọi thay đổi đối với các kiểu này sẽ được tự động phản ánh trên TOC trong tài liệu. Ví dụ bên dưới thay đổi thuộc tính định dạng được sử dụng trong kiểu `TOC` cấp đầu tiên.
...
Bằng cách sử dụng lớp [Style](https://reference.aspose.com/words/python-net/aspose.words/style/) được truy xuất cho cấp `TOC` cụ thể mà bạn muốn sửa đổi, bạn cũng có thể sửa đổi cách chúng xuất hiện trong tài liệu. Để thay đổi cách hiển thị này, trước tiên [Style.paragraph_format](https://reference.aspose.com/words/python-net/aspose.words/style/paragraph_format/) phải được gọi để truy xuất định dạng đoạn văn cho kiểu đó. Từ đó, các điểm dừng tab có thể được truy xuất bằng cách gọi [ParagraphFormat.tab_stops](https://reference.aspose.com/words/python-net/aspose.words/paragraphformat/tab_stops/) và điểm dừng tab thích hợp được sửa đổi. Sử dụng kỹ thuật tương tự này, bản thân tab có thể được di chuyển hoặc xóa cùng nhau. Ví dụ dưới đây cho thấy cách sửa đổi vị trí của điểm dừng tab bên phải trong các đoạn văn liên quan đến `TOC`.
...
Có thể xóa mục lục khỏi tài liệu bằng cách xóa tất cả các nút được tìm thấy giữa nút [FieldStart](https://reference.aspose.com/words/python-net/aspose.words.fields/fieldstart/) và [FieldEnd](https://reference.aspose.com/words/python-net/aspose.words.fields/fieldend/) của trường `TOC`. Đoạn mã dưới đây chứng minh điều này. Việc loại bỏ trường `TOC` đơn giản hơn trường thông thường vì chúng tôi không theo dõi các trường lồng nhau. Thay vào đó, chúng tôi kiểm tra nút [FieldEnd](https://reference.aspose.com/words/python-net/aspose.words.fields/fieldend/) có thuộc loại [FieldType.FIELD_TOC](https://reference.aspose.com/words/python-net/aspose.words.fields/fieldtype/#field_toc), có nghĩa là chúng tôi đã gặp phải phần cuối của TOC hiện tại. Kỹ thuật này có thể được sử dụng trong trường hợp này mà không cần lo lắng về bất kỳ trường lồng nhau nào vì chúng ta có thể giả định rằng mọi tài liệu được định dạng đúng cách sẽ không có trường `TOC` lồng nhau hoàn toàn bên trong một trường `TOC` khác.

Đầu tiên, các nút [FieldStart](https://reference.aspose.com/words/python-net/aspose.words.fields/fieldstart/) của mỗi `TOC` được thu thập và lưu trữ. Sau đó, `TOC` được chỉ định sẽ được liệt kê để tất cả các nút trong trường được truy cập và lưu trữ. Các nút sau đó được loại bỏ khỏi tài liệu. Mẫu Below code trình bày cách xóa `TOC` được chỉ định khỏi tài liệu.
