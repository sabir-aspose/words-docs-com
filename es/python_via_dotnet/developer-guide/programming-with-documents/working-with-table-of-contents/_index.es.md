---
title: Trabajar con tabla de contenidos
second_title: Aspose.Words para Python via .NET
articleTitle: Trabajar con tabla de contenidos
linktitle: Trabajar con tabla de contenidos
description: "Inserte y administre la 'Tabla de contenido' en un documento usando Python."
type: docs
weight: 170
...
## Insertar una tabla de contenido mediante programación

Puede insertar un campo `TOC` (tabla de contenido) en el documento en la posición actual llamando al método [DocumentBuilder.insert_table_of_contents](https://reference.aspose.com/words/python-net/aspose.words/documentbuilder/insert_table_of_contents/).

Una tabla de contenido en un documento de Word se puede crear de varias maneras y formatearse usando una variedad de opciones. Los cambios de campo que pasa al método controlan la forma en que se construye y muestra la tabla en su documento.
...
El código muestra la nueva tabla de contenido que se inserta en un documento en blanco. Luego, la clase [DocumentBuilder](https://reference.aspose.com/words/python-net/aspose.words/documentbuilder/) se usa para insertar algún formato de contenido de muestra con los estilos de encabezado apropiados que se usan para marcar el contenido que se incluirá en el TOC. Las siguientes líneas luego completan el `TOC` actualizando los campos y el diseño de página del documento.
...
La primera llamada a [Document.update_fields](https://reference.aspose.com/words/python-net/aspose.words/document/update_fields/) creará el `TOC`, se completarán todas las entradas de texto y el `TOC` aparecerá casi completo. Lo único que falta son los números de página que por ahora se muestran con "?". La segunda llamada a [Document.update_page_layout](https://reference.aspose.com/words/python-net/aspose.words/document/update_page_layout/) creará el diseño del documento en la memoria. Esto debe hacerse para recopilar los números de página de las entradas. Los números de página correctos calculados a partir de esta llamada luego se insertan en el TOC.
...
<p>Los propios campos `TC` también pueden tener varios interruptores configurados. Estos son:</p>
...
Puede insertar un nuevo campo TC en la posición actual del [DocumentBuilder](https://reference.aspose.com/words/python-net/aspose.words/documentbuilder/) llamando al método [DocumentBuilder.insert_field](https://reference.aspose.com/words/python-net/aspose.words/documentbuilder/insert_field/) y especificando el nombre del campo como "TC" junto con los modificadores que sean necesarios. El siguiente ejemplo muestra cómo insertar un campo `TC` en el documento usando [DocumentBuilder](https://reference.aspose.com/words/python-net/aspose.words/documentbuilder/).
...
El formato de las entradas en `TOC` no utiliza los estilos originales de las entradas marcadas, sino que cada nivel se formatea utilizando un estilo `TOC` equivalente. Por ejemplo, el primer nivel del `TOC` está formateado con el estilo **TOC1**, el segundo nivel con el estilo **TOC2**, etc. Esto significa que para cambiar el aspecto del `TOC` se deben modificar estos estilos. En Aspose.Words, estos estilos están representados desde [StyleIdentifier.TOC1](https://reference.aspose.com/words/python-net/aspose.words/styleidentifier/#toc1) independiente de la configuración regional hasta [StyleIdentifier.TOC9](https://reference.aspose.com/words/python-net/aspose.words/styleidentifier/#toc9) y se pueden recuperar de la colección [Document.styles](https://reference.aspose.com/words/python-net/aspose.words/documentbase/styles/) utilizando estos identificadores.

Una vez que se ha recuperado el estilo apropiado del documento, se puede modificar el formato de este estilo. Cualquier cambio en estos estilos se reflejará automáticamente en los TOC del documento. El siguiente ejemplo cambia una propiedad de formato utilizada en el estilo `TOC` de primer nivel.
...
Usando la clase [Style](https://reference.aspose.com/words/python-net/aspose.words/style/) recuperada para el nivel `TOC` particular que desea modificar, también puede modificar cómo aparecen en el documento. Para cambiar cómo aparece esto, primero se debe llamar a [Style.paragraph_format](https://reference.aspose.com/words/python-net/aspose.words/style/paragraph_format/) para recuperar el formato de párrafo para el estilo. A partir de aquí, las tabulaciones se pueden recuperar llamando a [ParagraphFormat.tab_stops](https://reference.aspose.com/words/python-net/aspose.words/paragraphformat/tab_stops/) y modificando la tabulación adecuada. Usando esta misma técnica, la pestaña se puede mover o quitar por completo. El siguiente ejemplo muestra cómo modificar la posición de la tabulación derecha en párrafos relacionados con `TOC`.
...
Se puede eliminar una tabla de contenido del documento eliminando todos los nodos que se encuentran entre el nodo [FieldStart](https://reference.aspose.com/words/python-net/aspose.words.fields/fieldstart/) y [FieldEnd](https://reference.aspose.com/words/python-net/aspose.words.fields/fieldend/) del campo `TOC`. El siguiente código demuestra esto. La eliminación del campo `TOC` es más sencilla que la de un campo normal, ya que no realizamos un seguimiento de los campos anidados. En lugar de eso, verificamos que el nodo [FieldEnd](https://reference.aspose.com/words/python-net/aspose.words.fields/fieldend/) sea de tipo [FieldType.FIELD_TOC](https://reference.aspose.com/words/python-net/aspose.words.fields/fieldtype/#field_toc), lo que significa que hemos encontrado el final del TOC actual. Esta técnica se puede utilizar en este caso sin preocuparse por los campos anidados, ya que podemos suponer que cualquier documento formado correctamente no tendrá ningún campo `TOC` completamente anidado dentro de otro campo `TOC`.

En primer lugar se recopilan y almacenan los nodos [FieldStart](https://reference.aspose.com/words/python-net/aspose.words.fields/fieldstart/) de cada `TOC`. Luego, el `TOC` especificado se enumera para que todos los nodos dentro del campo sean visitados y almacenados. Luego, los nodos se eliminan del documento. El ejemplo Below code demuestra cómo eliminar un `TOC` específico de un documento.
...
Si desea extraer una tabla de contenido de cualquier documento de Word, puede utilizar el siguiente ejemplo de código.
