...
透過使用我們的 [Free online converter](https://products.aspose.app/words/conversion) 工具來測試轉換功能。
...
"有關載入、轉換和儲存文件的詳細資訊，見 [Loading, Saving and Converting](/words/cpp/loading-saving-and-converting/) 節。"

## 高忠度

Aspose.Words的主要目標之一是提供文書格式間的高忠實度轉換。 我們使用高保真度來描述那些輸出文件保留原始文件的所有內容與格式的轉換。
...
在這種情況下， Aspose.Words 有高忠實度轉換標記，而輸出文件將與透過 Microsoft Word 轉換後的文件相同。
...
可調整的伺服器端情境使用 Aspose.Words 主要是因為它的簡單且乾淨的設計 – Aspose.Words 支援多線性。[Document](https://reference.aspose.com/words/net/aspose.words/document/) 的物件彼此獨立，只要只有一線可以一次修改一張文件（任何集合類別的典型要求），多個線就可以同時處理任意數量的文件。
