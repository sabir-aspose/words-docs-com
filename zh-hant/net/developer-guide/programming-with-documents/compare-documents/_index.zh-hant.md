...
透過比較，文件可確定為等於或不等于。 等號文件這個詞表示比較方法不能將更改表示為修訂。 這意味著文件文字與格式文字是一樣的。 但是文件間可以有其他的差異。 例如，Microsoft Word只支援格式版本為样式，而您無法表現样式插入/删除。 所以文件可以有不同的風格集，而**Compare**方法仍然產生零個修訂。
