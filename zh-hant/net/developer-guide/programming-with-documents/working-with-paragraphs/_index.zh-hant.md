...
邊界集和每個獨立的邊界都有類似的屬性，例如顏色、線條樣式、線條寬度、從文字的距離以及可選的陰影。 他們被同名的屬性所代表。 您可以透過結合屬性值來取得不同邊框類型。 此外，**BorderCollection** 和 **Border** 物件允許您透過呼叫 [ClearFormatting](https://reference.aspose.com/words/net/aspose.words/border/clearformatting/) 方法將這些值重設為預設值。
