...
在傳送一個檔案到客戶端瀏覽器時，請使用一個特別的超載來傳入四個參數：檔案名稱、儲存格式、儲存類型，及一個 ASP.NET HttpResponse 物件。 該文件會如何呈現給使用者由 [ContentDisposition](https://reference.aspose.com/words/net/aspose.words/contentdisposition/) 列舉表示，決定了送交到瀏覽器的文件會提供一個直接在瀏覽器中打開或在與檔案拡張名相關聯的應用程式中打開選項。
