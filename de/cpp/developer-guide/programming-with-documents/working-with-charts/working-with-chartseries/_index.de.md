---
title: Arbeiten mit ChartSeries in C++
second_title: Aspose.Words für C++
articleTitle: Arbeiten mit ChartSeries
linktitle: Arbeiten mit ChartSeries
description: "Diagrammserienabstimmung mit C++."
type: docs
weight: 40
ai_search_scope: words_cpp
ai_search_endpoint: "https://docsearch.api.aspose.cloud/ask"
ai_search_fast_endpoint: "https://docsearch.api.aspose.cloud/search"
url: /de/cpp/working-with-chartseries/
timestamp: 2024-01-27-14-07-04
---

Mit Aspose.Words können Benutzer auf verschiedene Arten mit ChartSeriesCollection arbeiten.

## Arbeiten mit ChartSeriesCollection des Diagramms

Schauen wir uns die Sammlung [ChartSeries](https://reference.aspose.com/words/cpp/aspose.words.drawing.charts/chartseries/) an. Alle Diagrammserien sind über [ChartSeriesCollection](https://reference.aspose.com/words/cpp/aspose.words.drawing.charts/chartseriescollection/) verfügbar:

{{< gist "aspose-words-gists" "d55d8631947d283b1f0da99afa06c492" "cpp-Programming-Documents-Charts-CreateColumnChart-ChartSeriesCollection.cpp" >}}


## Arbeiten mit einer einzelnen ChartSeries -Klasse

So arbeiten Sie mit einer bestimmten Serie:

{{< gist "aspose-words-gists" "d55d8631947d283b1f0da99afa06c492" "cpp-Programming-Documents-Charts-WorkWithSingleChartSeries-WorkWithSingleChartSeries.cpp" >}}

Bitte sehen Sie das Ergebnis unten:

![work-with-chartseries-aspose-words-cpp-1](working-with-chartseries-1.png)


Alle einzelnen [ChartSeries](https://reference.aspose.com/words/cpp/aspose.words.drawing.charts/chartseries/) haben Standardoptionen für [ChartDataPoint](https://reference.aspose.com/words/cpp/aspose.words.drawing.charts/chartseries/get_datapoints/). Bitte versuchen Sie, sie mit dem folgenden Code zu ändern:

{{< gist "aspose-words-gists" "d55d8631947d283b1f0da99afa06c492" "cpp-Programming-Documents-Charts-WorkWithSingleChartSeries-ChartDataPoint.cpp" >}}

![work-with-chartseries-aspose-words-cpp-2](working-with-chartseries-2.png)

## So arbeiten Sie mit einzelnen ChartDataPoint von `ChartSeries`

Mit [ChartDataPoint](https://reference.aspose.com/words/cpp/aspose.words.drawing.charts/chartdatapoint/) können Sie die Formatierung eines einzelnen Datenpunkts der Diagrammreihe anpassen:

{{< gist "aspose-words-gists" "d55d8631947d283b1f0da99afa06c492" "cpp-Programming-Documents-Charts-WorkWithSingleChartDataPoint-WorkWithSingleChartDataPoint.cpp" >}}

Bitte sehen Sie das Ergebnis unten:

![work-with-chartseries-aspose-words-cpp-3](working-with-chartseries-3.png)
